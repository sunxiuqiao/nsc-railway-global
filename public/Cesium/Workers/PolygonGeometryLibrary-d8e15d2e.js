...
define([
...
], function(e, I, x, E, y, A, P, _, d, G, L, M, v, D) {
  'use strict';
  function S() {
      ...
      packPolygonHierarchy: function(e, t, r) {
        for (var i = [e]; 0 < i.length; ) {
          ...
          if (I.defined(n)) {
            ...
            if (I.defined(o))
              for (var l = o.length, h = 0; h < l; ++h) i.push(o[h]);
          }
        }
        return r;
      },
  ...
  R.subdivideLineCount = function(e, t, r) {
    var i = E.Cartesian3.distance(e, t) / r,
      n = Math.max(0, Math.ceil(x.CesiumMath.log2(i)));
    return Math.pow(2, n);
  };
  var m = new E.Cartographic(),
    C = new E.Cartographic(),
    b = new E.Cartographic(),
    w = new E.Cartesian3();
  (R.subdivideRhumbLineCount = function(e, t, r, i) {
    var n = e.cartesianToCartographic(t, m),
      a = e.cartesianToCartographic(r, C),
      o = new v.EllipsoidRhumbLine(n, a, e).surfaceDistance / i,
      s = Math.max(0, Math.ceil(x.CesiumMath.log2(o)));
    return Math.pow(2, s);
  }),
    (R.subdivideLine = function(e, t, r, i) {
      ...
      for (var u, l, h, c, f = 0, p = 0; p < n; p++) {
        var d =
          ...
          (h = p * o),
          (c = a),
          E.Cartesian3.subtract(l, u, g),
          E.Cartesian3.multiplyByScalar(g, h / c, g),
          E.Cartesian3.add(u, g, g),
          [g.x, g.y, g.z]);
    ...
    (R.subdivideRhumbLine = function(e, t, r, i, n) {
      var a = e.cartesianToCartographic(t, m),
        ...
        s = new v.EllipsoidRhumbLine(a, o, e),
        u = s.surfaceDistance / i,
        l = Math.max(0, Math.ceil(x.CesiumMath.log2(u))),
        h = Math.pow(2, l),
      ...
      for (var p = 0, d = 0; d < h; d++) {
        var y = s.interpolateUsingSurfaceDistance(d * c, b),
          g = e.cartographicToCartesian(y, w);
  ...
  (R.scaleToGeodeticHeightExtruded = function(e, t, r, i, n) {
    ...
    if (
      ...
      for (
        var l = e.attributes.position.values, h = l.length / 2, c = 0;
        c < h;
        c += 3
      )
        E.Cartesian3.fromArray(l, c, s),
      ...
      for (s.enqueue(e); 0 !== s.length; ) {
        ...
        if (t)
          for (a = l.length, i = 0; i < a; i++)
            r.scaleToGeodeticSurface(l[i], l[i]);
        if (
          !(
          ...
          for (i = 0; i < h; i++) {
            var c = u.holes[i],
              f = c.positions;
            if (t)
              for (a = f.length, n = 0; n < a; ++n)
                r.scaleToGeodeticSurface(f[n], f[n]);
            if (
              !(
              ...
              var p = 0;
              for (
                I.defined(c.holes) && (p = c.holes.length), n = 0;
                n < p;
                n++
              )
                s.enqueue(c.holes[n]);
    ...
    (R.polygonsFromHierarchy = function(e, t, r, i) {
      ...
      for (o.enqueue(e); 0 !== o.length; ) {
        var s,
          ...
          c = l.holes;
        if (r)
          for (u = h.length, s = 0; s < u; s++)
            i.scaleToGeodeticSurface(h[s], h[s]);
        if (
          !(
            (h = L.arrayRemoveDuplicates(h, E.Cartesian3.equalsEpsilon, !0))
              .length < 3
          )
        ) {
          var f = t(h);
          if (I.defined(f)) {
            ...
            d === D.WindingOrder.CLOCKWISE &&
              (f.reverse(), (h = h.slice().reverse()));
            var y,
              g = h.slice(),
            ...
            for (s = 0; s < v; s++) {
              var C = c[s],
                b = C.positions;
              if (r)
                for (u = b.length, y = 0; y < u; ++y)
                  i.scaleToGeodeticSurface(b[y], b[y]);
              if (
                !(
                ...
                var w = t(b);
                if (I.defined(w)) {
                  (d = D.PolygonPipeline.computeWindingOrder2D(w)) ===
                  ...
                  var T = 0;
                  for (
                    I.defined(C.holes) && (T = C.holes.length), y = 0;
                    y < T;
                    y++
                  )
                    o.enqueue(C.holes[y]);
            ...
            n.push({ outerRing: h, holes: m }),
              a.push({ positions: g, positions2D: f, holes: p });
          }
        }
      }
  ...
  (R.computeBoundingRectangle = function(e, t, r, i, n) {
    for (
      var a = y.Quaternion.fromAxisAngle(e, i, B),
        o = y.Matrix3.fromQuaternion(a, H),
  ...
  (R.computeWallGeometry = function(e, t, r, i, n) {
    var a,
      ...
      c = 0;
    if (i)
      for (o = 3 * h * 2, a = new Array(2 * o), s = 0; s < h; s++)
        (u = e[s]),
          (l = e[(s + 1) % h]),
          (a[c] = a[c + o] = u.x),
          (a[++c] = a[c + o] = u.y),
      ...
      var f = x.CesiumMath.chordLength(r, t.maximumRadius),
        p = 0;
      if (n === M.ArcType.GEODESIC)
        for (s = 0; s < h; s++)
          p += R.subdivideLineCount(e[s], e[(s + 1) % h], f);
      else if (n === M.ArcType.RHUMB)
        for (s = 0; s < h; s++)
          p += R.subdivideRhumbLineCount(t, e[s], e[(s + 1) % h], f);
      for (o = 3 * (p + h), a = new Array(2 * o), s = 0; s < h; s++) {
        var d;
        (u = e[s]),
          (l = e[(s + 1) % h]),
          n === M.ArcType.GEODESIC
            ? (d = R.subdivideLine(u, l, f, k))
            : n === M.ArcType.RHUMB &&
              (d = R.subdivideRhumbLine(t, u, l, f, k));
        for (var y = d.length, g = 0; g < y; ++g, ++c)
          (a[c] = d[g]), (a[c + o] = d[g]);
        (a[c] = l.x),
          (a[c + o] = l.x),
          (a[++c] = l.y),
    ...
    h = a.length;
    var v = G.IndexDatatype.createTypedArray(h / 3, h - 6 * e.length),
      m = 0;
    for (h /= 6, s = 0; s < h; s++) {
      var C = s,
        b = C + 1,
        w = C + h,
        T = w + 1;
      (u = E.Cartesian3.fromArray(a, 3 * C, z)),
        (l = E.Cartesian3.fromArray(a, 3 * b, W)),
        E.Cartesian3.equalsEpsilon(
          u,
          l,
