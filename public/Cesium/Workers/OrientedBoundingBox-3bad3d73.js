...
define([
...
], function(a, q, t, X, j, A, Z, G) {
  'use strict';
  function T(a, t) {
    (this.center = j.Cartesian3.clone(q.defaultValue(a, j.Cartesian3.ZERO))),
      (this.halfAxes = A.Matrix3.clone(q.defaultValue(t, A.Matrix3.ZERO)));
  }
  (T.packedLength = j.Cartesian3.packedLength + A.Matrix3.packedLength),
    (T.pack = function(a, t, e) {
      return (
        (e = q.defaultValue(e, 0)),
        j.Cartesian3.pack(a.center, t, e),
        A.Matrix3.pack(a.halfAxes, t, e + j.Cartesian3.packedLength),
        t
      );
    }),
    (T.unpack = function(a, t, e) {
      return (
        (t = q.defaultValue(t, 0)),
        q.defined(e) || (e = new T()),
        j.Cartesian3.unpack(a, t, e.center),
        A.Matrix3.unpack(a, t + j.Cartesian3.packedLength, e.halfAxes),
        e
      );
    });
  var R = new j.Cartesian3(),
    I = new j.Cartesian3(),
    E = new j.Cartesian3(),
    L = new j.Cartesian3(),
    z = new j.Cartesian3(),
    S = new j.Cartesian3(),
    U = new A.Matrix3(),
    V = { unitary: new A.Matrix3(), diagonal: new A.Matrix3() };
  T.fromPoints = function(a, t) {
    if ((q.defined(t) || (t = new T()), !q.defined(a) || 0 === a.length))
      return (t.halfAxes = A.Matrix3.ZERO), (t.center = j.Cartesian3.ZERO), t;
    var e,
      n = a.length,
      r = j.Cartesian3.clone(a[0], R);
    ...
    var s,
      ...
      l = 0;
    for (e = 0; e < n; e++)
      (o += (s = j.Cartesian3.subtract(a[e], r, I)).x * s.x),
        (C += s.x * s.y),
        (c += s.x * s.z),
        (u += s.y * s.y),
        (d += s.y * s.z),
    ...
    (h[0] = o),
      ...
      (h[7] = d),
      (h[8] = l);
    var x = A.Matrix3.computeEigenDecomposition(h, V),
      M = A.Matrix3.clone(x.unitary, t.halfAxes),
      m = A.Matrix3.getColumn(M, 0, L),
      f = A.Matrix3.getColumn(M, 1, z),
      p = A.Matrix3.getColumn(M, 2, S),
      g = -Number.MAX_VALUE,
      w = -Number.MAX_VALUE,
      y = -Number.MAX_VALUE,
      O = Number.MAX_VALUE,
    ...
    for (e = 0; e < n; e++)
      (s = a[e]),
        ...
        (O = Math.min(j.Cartesian3.dot(m, s), O)),
        (b = Math.min(j.Cartesian3.dot(f, s), b)),
        (P = Math.min(j.Cartesian3.dot(p, s), P));
    (m = j.Cartesian3.multiplyByScalar(m, 0.5 * (O + g), m)),
      (f = j.Cartesian3.multiplyByScalar(f, 0.5 * (b + w), f)),
    ...
    return (
      ...
      A.Matrix3.multiplyByScale(t.halfAxes, v, t.halfAxes),
      t
    );
  };
  ...
  function F(a, t, e, n, r, i, s, o, C, c, u) {
    q.defined(u) || (u = new T());
    var d = u.halfAxes;
    A.Matrix3.setColumn(d, 0, t, d),
      A.Matrix3.setColumn(d, 1, e, d),
      A.Matrix3.setColumn(d, 2, n, d);
    var l = M;
    ...
    var x = u.center;
    return (
      (l = A.Matrix3.multiplyByVector(d, l, l)),
      j.Cartesian3.add(a, l, x),
      A.Matrix3.multiplyByScale(d, h, d),
      u
    );
  }
  ...
  (T.fromRectangle = function(a, t, e, n, r) {
    ...
    if (
      ((t = q.defaultValue(t, 0)),
      (e = q.defaultValue(e, 0)),
      (n = q.defaultValue(n, j.Ellipsoid.WGS84)),
      a.width <= X.CesiumMath.PI)
    ) {
      ...
      var M = l.longitude,
        ...
        f = j.Cartographic.fromRadians(M, a.north, e, J),
        ...
        y = j.Cartographic.fromRadians(M, a.south, e, aa),
        O = n.cartographicToCartesian(f, ta),
        b = n.cartographicToCartesian(p, ea),
        P = n.cartographicToCartesian(g, na),
        N = n.cartographicToCartesian(w, ra),
        v = n.cartographicToCartesian(y, ia),
        A = x.projectPointToNearestOnPlane(O, sa),
        T = x.projectPointToNearestOnPlane(b, oa),
        R = x.projectPointToNearestOnPlane(P, Ca),
        I = x.projectPointToNearestOnPlane(N, ca),
      ...
      return (
        ...
        (N = n.cartographicToCartesian(w, ra)),
        (c = Math.min(
          Z.Plane.getPointDistance(d, b),
          Z.Plane.getPointDistance(d, N),
        )),
        (u = e),
        F(x.origin, x.xAxis, x.yAxis, x.zAxis, i, s, o, C, c, u, r)
      );
    }
    var L = 0 < a.south,
      z = a.north < 0,
      S = L ? a.south : z ? a.north : 0,
      U = j.Rectangle.center(a, Y).longitude,
      V = j.Cartesian3.fromRadians(U, S, e, n, da);
    V.z = 0;
    var B =
        Math.abs(V.x) < X.CesiumMath.EPSILON10 &&
        Math.abs(V.y) < X.CesiumMath.EPSILON10
          ? j.Cartesian3.UNIT_X
          : j.Cartesian3.normalize(V, la),
      _ = j.Cartesian3.UNIT_Z,
      k = j.Cartesian3.cross(B, _, ha);
    d = Z.Plane.fromPointNormal(V, B, ga);
    var W = j.Cartesian3.fromRadians(U + X.CesiumMath.PI_OVER_TWO, S, e, n, xa);
    (i = -(s = j.Cartesian3.dot(Z.Plane.projectPointOntoPlane(d, W, Ma), k))),
    ...
    return F(
      ...
      o,
      C,
      (c = Z.Plane.getPointDistance(d, D)),
      (u = 0),
      r,
    );
  }),
    (T.clone = function(a, t) {
      if (q.defined(a))
        return q.defined(t)
          ? (j.Cartesian3.clone(a.center, t.center),
            A.Matrix3.clone(a.halfAxes, t.halfAxes),
            t)
          : new T(a.center, a.halfAxes);
    }),
    (T.intersectPlane = function(a, t) {
      var e = a.center,
        ...
        C =
          Math.abs(
            i * r[A.Matrix3.COLUMN0ROW0] +
              s * r[A.Matrix3.COLUMN0ROW1] +
              o * r[A.Matrix3.COLUMN0ROW2],
          ) +
          Math.abs(
            i * r[A.Matrix3.COLUMN1ROW0] +
              s * r[A.Matrix3.COLUMN1ROW1] +
              o * r[A.Matrix3.COLUMN1ROW2],
          ) +
          Math.abs(
            i * r[A.Matrix3.COLUMN2ROW0] +
              s * r[A.Matrix3.COLUMN2ROW1] +
              o * r[A.Matrix3.COLUMN2ROW2],
          ),
  ...
  T.distanceSquaredTo = function(a, t) {
    var e = j.Cartesian3.subtract(t, a.center, M),
      n = a.halfAxes,
      r = A.Matrix3.getColumn(n, 0, x),
      i = A.Matrix3.getColumn(n, 1, f),
      s = A.Matrix3.getColumn(n, 2, p),
      o = j.Cartesian3.magnitude(r),
      C = j.Cartesian3.magnitude(i),
      c = j.Cartesian3.magnitude(s);
    j.Cartesian3.normalize(r, r),
      j.Cartesian3.normalize(i, i),
      j.Cartesian3.normalize(s, s);
    var u = h;
    (u.x = j.Cartesian3.dot(e, r)),
      (u.y = j.Cartesian3.dot(e, i)),
      (u.z = j.Cartesian3.dot(e, s));
    var d,
      l = 0;
    return (
  ...
  T.computePlaneDistances = function(a, t, e, n) {
    ...
    var r = Number.POSITIVE_INFINITY,
      ...
      s = a.center,
      o = a.halfAxes,
      C = A.Matrix3.getColumn(o, 0, x),
      c = A.Matrix3.getColumn(o, 1, f),
      u = A.Matrix3.getColumn(o, 2, p),
    ...
    (T.equals = function(a, t) {
      return (
        ...
        (q.defined(a) &&
          ...
          A.Matrix3.equals(a.halfAxes, t.halfAxes))
      );
    }),
