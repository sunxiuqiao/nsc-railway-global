...
define([
...
], function(Te, e, pe, fe, Ee, t, r, n, ve, ye, i, o, Ne, a, we, xe, Me) {
  'use strict';
  function be() {
    ...
    return (
      Te.defined(n) ||
        p(
          ...
          t,
          (n =
            e * t < pe.CesiumMath.SIXTY_FOUR_KILOBYTES
  ...
  (be.getRegularGridAndSkirtIndicesAndEdgeIndices = function(e, t) {
    ...
    (be.addSkirtIndices = function(e, t, r, n, i, o, a) {
      var s = i;
      (a = d(e, s, o, a)),
        (a = d(t, (s += e.length), o, a)),
        (a = d(r, (s += t.length), o, a)),
        d(n, (s += r.length), o, a);
    }),
    (be.heightmapTerrainQuality = 0.25),
    (be.getEstimatedLevelZeroGeometricErrorForAHeightmap = function(e, t, r) {
      return (
        (2 * e.maximumRadius * Math.PI * be.heightmapTerrainQuality) / (t * r)
      );
    }),
    (be.prototype.requestTileGeometry =
      e.DeveloperError.throwInstantiationError),
    (be.prototype.getLevelMaximumGeometricError =
      e.DeveloperError.throwInstantiationError),
    (be.prototype.getTileDataAvailable =
      e.DeveloperError.throwInstantiationError),
    (be.prototype.loadTileDataAvailability =
      e.DeveloperError.throwInstantiationError);
  var Ce = 32767,
    Se = new fe.Cartesian3(),
    Ae = new fe.Cartesian3(),
    Pe = new fe.Cartesian3(),
    We = new fe.Cartographic(),
    De = new fe.Cartesian2(),
  ...
  function Ve(e, t, r, n, i, o, a, s, d) {
    ...
    for (var m = e.length, g = 0; g < m; ++g) {
      var T = e[g],
        p = r[T],
        f = n[T];
      (We.longitude = pe.CesiumMath.lerp(I, l, f.x)),
        (We.latitude = pe.CesiumMath.lerp(h, u, f.y)),
        (We.height = p - t);
  ...
  function _e(e, t, r, n, i, o, a, s, d, c, u, h, l, I, m) {
    ...
    for (var v = r.length, y = 0; y < v; ++y) {
      var N = r[y],
        w = i[N],
        x = o[N];
      (We.longitude = pe.CesiumMath.lerp(E, f, x.x) + I),
        (We.latitude = pe.CesiumMath.lerp(p, T, x.y) + m),
        (We.height = w - c);
      var M,
        b = s.cartographicToCartesian(We, Se);
      if (g) {
        var C = 2 * N;
        if (((De.x = a[C]), (De.y = a[1 + C]), 1 !== u)) {
          ...
          Ee.Matrix4.multiplyByPointAsVector(P, S, S),
            (S.z *= u),
            fe.Cartesian3.normalize(S, S),
            Ee.Matrix4.multiplyByPointAsVector(A, S, S),
            fe.Cartesian3.normalize(S, S),
            ve.AttributeCompression.octEncode(S, De);
        }
      }
      n.hasWebMercatorT &&
        (M =
          (Ne.WebMercatorProjection.geodeticLatitudeToMercatorAngle(
            ...
            h) *
          l),
        (t = n.encode(e, t, b, x, We.height, De, M));
    }
  }
  function He(e, t) {
    var r;
  ...
  return a(function(e, t) {
    ...
    d &&
      ((r = Ne.WebMercatorProjection.geodeticLatitudeToMercatorAngle(h)),
      (n =
        1 / (Ne.WebMercatorProjection.geodeticLatitudeToMercatorAngle(I) - r)));
    var y = i.subarray(0, o),
      ...
      S = d ? new Array(o) : [],
      A = Ae;
    (A.x = Number.POSITIVE_INFINITY),
      (A.y = Number.POSITIVE_INFINITY),
      (A.z = Number.POSITIVE_INFINITY);
    var P = Pe;
    (P.x = Number.NEGATIVE_INFINITY),
      (P.y = Number.NEGATIVE_INFINITY),
    ...
    for (
      ...
      var V = y[k],
        ...
        H = V / Ce,
        O = _ / Ce,
        G = pe.CesiumMath.lerp(T, p, w[k] / Ce);
      (We.longitude = pe.CesiumMath.lerp(u, l, H)),
        (We.latitude = pe.CesiumMath.lerp(h, I, O)),
        (We.height = G),
        (W = Math.min(We.longitude, W)),
        (D = Math.max(We.longitude, D)),
        (B = Math.min(We.latitude, B)),
        (F = Math.max(We.latitude, F));
      var Y = m.cartographicToCartesian(We);
      (M[k] = new fe.Cartesian2(H, O)),
        (b[k] = G),
        (C[k] = Y),
        d &&
          (S[k] =
            (Ne.WebMercatorProjection.geodeticLatitudeToMercatorAngle(
              ...
              r) *
            n),
        Ee.Matrix4.multiplyByPoint(v, Y, Se),
        fe.Cartesian3.minimumByComponent(Se, A, A),
        fe.Cartesian3.maximumByComponent(Se, P, P);
    }
    var z,
      R,
      L,
    ...
    1 !== g &&
      ((R = Ee.BoundingSphere.fromPoints(C)),
      (z = xe.OrientedBoundingBox.fromRectangle(c, T, p, m))),
      (1 !== g || T < 0) &&
        (L = new Me.EllipsoidalOccluder(
          m,
        ).computeHorizonCullingPointPossiblyUnderEllipsoid(f, C, T));
    var K = T;
    (K = Math.min(
      K,
      Ve(e.westIndices, e.westSkirtHeight, b, M, c, m, v, A, P),
    )),
      (K = Math.min(
        K,
        Ve(e.southIndices, e.southSkirtHeight, b, M, c, m, v, A, P),
      )),
      (K = Math.min(
        K,
        Ve(e.eastIndices, e.eastSkirtHeight, b, M, c, m, v, A, P),
      )),
      (K = Math.min(
        K,
        Ve(e.northIndices, e.northSkirtHeight, b, M, c, m, v, A, P),
      ));
    for (
      var X = new we.AxisAlignedBoundingBox(A, P, f),
        Z = new Me.TerrainEncoding(X, K, p, E, x, d),
        J = Z.getStride(),
      ...
      if (x) {
        ...
        if (((De.x = a[re]), (De.y = a[1 + re]), 1 !== g)) {
          var ne = ve.AttributeCompression.octDecode(De.x, De.y, Be),
            ie = Ee.Transforms.eastNorthUpToFixedFrame(C[te], m, ke),
          ...
          Ee.Matrix4.multiplyByPointAsVector(oe, ne, ne),
            ...
            fe.Cartesian3.normalize(ne, ne),
            ve.AttributeCompression.octEncode(ne, De);
        }
      }
      ee = Z.encode($, ee, C[te], M[te], b[te], De, S[te]);
    }
    ...
    var ce = 1e-4 * (D - W),
      ...
      ge = o * J;
    return (
      _e($, ge, U, Z, b, M, a, m, c, e.westSkirtHeight, g, r, n, he, 0),
      _e(
        $,
